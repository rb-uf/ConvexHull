...
void runTestAndPrint(vector<SimplePoint2D> p)
{
    ...
    ConvexHullMonotoneChain(Point2D(p)).print();
    cout << "Graham Scan" << endl;
    ConvexHullGrahamScan(Point2D(p)).print();
    cout << "Quickhull" << endl;
    ConvexHullQuickhull(Point2D(p)).print();
    cout << "Jarvis March" << endl;
    ConvexHullJarvisMarch(Point2D(p)).print();
    cout << "Divide and Conquer" << endl;
    ConvexHullDivideandConquer(Point2D(p)).print();
    cout << "Incremental" << endl;
    ConvexHullIncremental(Point2D(p)).print();
}
...
void runTestAndTime(vector<SimplePoint2D> p)
{
    ...
    start = getTime();
    Region2D r1 = ConvexHullMonotoneChain(Point2D(p));
    stop = getTime();
    cout << "MonotoneChain runtime:         " << (stop - start) << endl;

    start = getTime();
    Region2D r2 = ConvexHullGrahamScan(Point2D(p));
    stop = getTime();
    cout << "GrahamScan runtime:            " << (stop - start) << endl;

    start = getTime();
    Region2D r3 = ConvexHullQuickhull(Point2D(p));
    stop = getTime();
